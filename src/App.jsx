...
// ══════════════════════════════════════════════════════════════
const GEMINI_MODELS = [
  { id:"gemini-3.6-flash",      label:"Gemini 3.6 Flash",      badge:"KHUYẾN NGHỊ", color:"#34a853", note:"Bản GA ổn định — token-efficient, giá thấp, cân bằng tốt nhất cho dùng hàng ngày." },
  { id:"gemini-3.8-flash",      label:"Gemini 3.8 Flash",      badge:"MỚI NHẤT",    color:"#4285f4", note:"Flash thế hệ mới nhất — mạnh nhất cho audio phức tạp, giá introductory đến hết 2026." },
  { id:"gemini-3.5-flash-lite", label:"Gemini 3.5 Flash-Lite", badge:"RẺ NHẤT",     color:"#ffb300", note:"Latency thấp, chi phí thấp nhất — phù hợp ghi âm R/T ngắn tần suất cao." },
];
const DEFAULT_MODEL = "gemini-3.6-flash";
// Model 1.x/2.x đã bị Google shutdown (6/2026) — tự migrate localStorage cũ
const migrateModel = (m) => (!m || /^gemini-[12]\./.test(m)) ? DEFAULT_MODEL : m;
...
export default function App() {
  ...
  const [gemModel,   setGemModel]   = useState(()=>migrateModel(localStorage.getItem("as_gem_model")));
  ...
  const buildDSP = useCallback((stream,preset)=>{
    ...
    ctx.resume(); // CRITICAL: browser autoplay policy suspends AudioContext — without resume, DSP output is SILENT
  ...
  const startRec=async(isMeeting=false)=>{
    ...
    try {
      ...
      // Meeting: bitrate thấp hơn (32kbps đủ cho giọng nói) → giảm dung lượng + token khi audio dài
      const mr=new MediaRecorder(recStream,{mimeType:"audio/webm;codecs=opus",audioBitsPerSecond:isMeeting?32000:64000});
      ...
      timerRef.current=setInterval(()=>setRecTime(t=>{
        const nt=t+1;
        // Meeting: tự dừng ở 5 phút — audio dài hơn sẽ vượt token limit free tier của Gemini
        // (ghi họp dài = chia thành nhiều đoạn 5 phút, mỗi đoạn tự phiên dịch xong lại bấm ghi tiếp)
        if(isMeeting&&nt>=300){ setTimeout(()=>{ if(mediaRecRef.current?.state==="recording"){ mediaRecRef.current.stop(); setIsRec(false); clearInterval(timerRef.current); } },0); }
        return nt;
      }),1000);
  ...
  const gemProcess=async(blob,isMeeting)=>{
    ...
    try {
      ...
      // Gọi Gemini với retry tự động khi bị rate limit (429)
      let res, attempt=0;
      while(attempt<3){
        res=await fetch(gemEndpoint(),{
        ...
        attempt++;
        flash(`Rate limit — thử lại lần ${attempt}/3 sau ${attempt*5}s...`,"warn");
        await new Promise(r=>setTimeout(r,attempt*5000));
      }
      if(!res.ok){
        const e=await res.json().catch(()=>({}));
        const msg=e.error?.message||`HTTP ${res.status}`;
        if(msg.includes("free_tier")||res.status===429)
          throw new Error("Vượt hạn mức API. Thử: (1) đổi model Gemini 3.5 Flash-Lite ở Cài đặt, (2) ghi đoạn ngắn hơn, (3) đợi 1 phút rồi thử lại, (4) kiểm tra billing tại aistudio.google.com.");
        throw new Error(msg);
      }
          ...
          <div style={{animation:"slideIn 0.3s ease"}}>
            ...
            <Card style={{marginBottom:16}}>
              ...
              <div style={{display:"flex",justifyContent:"space-between",marginTop:8}}>
                ...
                <span style={{fontSize:11,fontFamily:MONO,color:recTime>=240&&isRec&&meetMode?C.amber:C.textDim}}>{isRec&&meetMode?`MTG ${fmtD(recTime)} / 05:00 (tự dừng)`:isProc?"GEMINI...":"STANDBY · MAX 5 PHÚT/ĐOẠN"}</span>
              </div>
            </Card>
